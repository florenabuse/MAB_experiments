        ...
        with open('information_rl.csv', 'w') as file:
            ...
            for episode in range(episode_num):
                ...
                while True:
                    ...
                    state_next, reward, test_auc, done, ft_imp = self.env.step(action)
                    ...
                    if done:
                        ...
                        if episode + 1 == episode_num:
                            with open('../results/output_rl.csv', 'a') as f:
                                f.write(
                                    f',{self.env.agg},RL,{self.env.folder},,{round(time_end - time_start, 2)},{round(time_end - time_start, 2)},,{self.env.cur_score},0,"{feature_importance}","{list(self.env.current_training_set.columns)}",,,\n')
                            return
                        break
